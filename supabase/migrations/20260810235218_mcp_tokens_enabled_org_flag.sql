-- Permite al owner de una organización desactivar por completo la creación
-- (y el uso) de tokens MCP para todos sus miembros, incluido él mismo.
-- Default true para no romper el comportamiento existente de ninguna org.
alter table public.organizations
  add column mcp_tokens_enabled boolean not null default true;

-- create_mcp_session ahora rechaza la creación de tokens nuevos si el flag
-- está apagado para la organización del usuario autenticado.
...
begin
  ...
  select mcp_tokens_enabled into v_mcp_enabled from organizations where id = v_tenant_id;
  if not coalesce(v_mcp_enabled, true) then
    raise exception 'la organización deshabilitó la creación de tokens MCP';
  end if;
...
-- Las 4 RPCs de uso del token también revalidan el flag en cada llamada,
-- para que tokens YA emitidos dejen de funcionar apenas el owner apaga el
-- flag, sin esperar a que expiren o se revoquen manualmente.
