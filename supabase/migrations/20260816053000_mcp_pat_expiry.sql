-- MCP personal access tokens never expired — only revoked_at was checked.
-- A leaked token stayed valid indefinitely. Add an optional expires_at
-- (null = existing tokens keep working unexpired, since we can't retroactively
-- assign a meaningful expiry) and default new tokens to a 90-day lifetime.
alter table mcp_sessions add column expires_at timestamptz;
