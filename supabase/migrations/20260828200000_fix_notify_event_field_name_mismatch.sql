-- Critical fix: the webhook body used the key 'eventType', but
-- notify.ts's validateEvent() (and the NotificationEvent type) require
-- 'type'. Every trigger-fired notification since the notify-event wiring
-- was added has been silently rejected by validateEvent (logged a
-- console.warn, no email sent, no failed_jobs row — the endpoint always
-- returns 200 by design, which is why this went undetected by the
-- earlier live verification: absence of an error was mistaken for
-- success). Found by an independent architecture-audit agent.
