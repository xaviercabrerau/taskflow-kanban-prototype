...
> **Nota de estado (2026-09-10):** este es un informe histórico del 2026-09-03. Verificado contra el código actual: sigue vigente — `BoardContext.tsx` ya no es un único `useMemo` monolítico (623 líneas, 3 `useMemo` separados), `src/lib/notifications/gmail.ts` y las dependencias `googleapis`/`google-auth-library` ya no existen (confirman el hallazgo #10/#11 y el arreglo de la auditoría anterior). El dominio `https://task.conto.ec` citado abajo sigue siendo la URL de producción real; si el proyecto se migra a otra cuenta ver [`MIGRACION.md`](MIGRACION.md).
